...
from shared.auth.keycloak import build_validator_from_settings
...
async def lifespan(app: FastAPI):
    log.info("service.startup", service="payment-service")
    ...
    producer = PaymentEventProducer(settings.KAFKA_BOOTSTRAP_SERVERS, "payment-service")
    try:
    ...
    # Keycloak validator
    app.state.keycloak_validator = build_validator_from_settings(settings)

    # Acquirer adapter
    if settings.ACQUIRER_MODE == "razorpay":
        from adapters.razorpay import RazorpayAdapter
        app.state.acquirer = RazorpayAdapter(settings.RAZORPAY_KEY_ID, settings.RAZORPAY_KEY_SECRET)
    else:
        from adapters.mock import MockAcquirerAdapter
        app.state.acquirer = MockAcquirerAdapter()

    log.info("service.ready", service="payment-service", acquirer=settings.ACQUIRER_MODE)
    yield
    ...
    log.info("service.shutdown", service="payment-service")
