...
class RequestIDMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        structlog.contextvars.bind_contextvars(request_id=request_id, service="transaction-service")
...
async def lifespan(app: FastAPI):
    log.info("service.startup", service="transaction-service")
    ...
    log.info("service.ready", service="transaction-service")
    yield
    await redis.aclose()
    await engine.dispose()
    log.info("service.shutdown", service="transaction-service")
