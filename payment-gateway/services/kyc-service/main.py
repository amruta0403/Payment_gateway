...
class RequestIDMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        structlog.contextvars.bind_contextvars(request_id=request_id, service="kyc-service")
...
async def lifespan(app: FastAPI):
    log.info("service.startup", service="kyc-service")
    ...
    log.info("service.ready", service="kyc-service")
    yield
    await redis.aclose()
    await engine.dispose()
    log.info("service.shutdown", service="kyc-service")

app = FastAPI(title="kyc-service", version="0.1.0", lifespan=lifespan,
