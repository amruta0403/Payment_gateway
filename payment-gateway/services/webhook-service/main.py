...
class RequestIDMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        structlog.contextvars.bind_contextvars(request_id=request_id, service="webhook-service")
...
async def lifespan(app: FastAPI):
    log.info("service.startup", service="webhook-service")
    ...
    log.info("service.ready", service="webhook-service")
    yield
    await redis.aclose()
    await engine.dispose()
    log.info("service.shutdown", service="webhook-service")
