...
class RequestIDMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        structlog.contextvars.bind_contextvars(request_id=request_id, service="netbanking-service")
...
async def lifespan(app: FastAPI):
    log.info("service.startup", service="netbanking-service")
    ...
    log.info("service.ready", service="netbanking-service")
    yield
    await redis.aclose()
    await engine.dispose()
    log.info("service.shutdown", service="netbanking-service")

app = FastAPI(title="netbanking-service", version="0.1.0", lifespan=lifespan,
