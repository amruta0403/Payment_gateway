...
class RequestIDMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        structlog.contextvars.bind_contextvars(request_id=request_id, service="reporting-service")
...
async def lifespan(app: FastAPI):
    log.info("service.startup", service="reporting-service")
    ...
    log.info("service.ready", service="reporting-service")
    yield
    await redis.aclose()
    await engine.dispose()
    log.info("service.shutdown", service="reporting-service")
